...
@pytest.mark.parametrize(
    ...
    [
        ...
        for test in [
            (4, 4096, 8, 128, 1, 1, 1, torch.float16),
            (4, 4096, 16, 128, 0.1, 0.1, 10, torch.float16),
            (4, 3407, 16, 128, 0.1, 1, 10, torch.float16),
            (4, 2048, 4, 256, 1, 0.1, 0.1, torch.float16),
            (4, 3396, 8, 192, 0.1, 0.1, 1, torch.float16),
            (2, 3536, 4, 128, 0.1, 0.1, 1, torch.float16),
            (4, 3333, 4, 256, 0.1, 0.1, 0.1, torch.float16),
            (4, 4001, 4, 256, 0.1, 0.1, 10, torch.float16),
        ]
    ]
)
def test_chunk_forward(
    ...
    ref, ht_ref = recurrent_gsa2_ref(
        q=q.clone(),
        k=k.clone(),
        v=v.clone(),
        w=F.normalize(w.clone(), p=2, dim=-1),
        beta=beta.clone(),
        g=g.clone(),
        scale_k=scale_k,
        scale_v=scale_v,
    ...
    assert_close('hk', ht_ref[0], ht_tri[0], 0.002)
    assert_close('hv', ht_ref[1], ht_tri[1], 0.002)
...
def test_chunk(
    ...
    ref, (ref_hkt, ref_hvt) = recurrent_gsa2_ref(
        q=q.clone(),
        k=k.clone(),
        v=v.clone(),
        w=F.normalize(w.clone(), p=2, dim=-1),
    ...
    tri, (tri_hkt, tri_hvt) = chunk_gsa2(
        q=q.clone(),
        k=k.clone(),
        v=v.clone(),
        w=F.normalize(w.clone(), p=2, dim=-1),
