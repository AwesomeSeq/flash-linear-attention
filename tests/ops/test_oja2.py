...
from fla.ops.oja2 import fused_recurrent_oja2
from fla.ops.oja2 import chunk_oja2
from fla.utils import assert_close, device, is_intel_alchemist



def recurrent_oja2_ref(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    beta: torch.Tensor,
...
def test_fused_recurrent(
    ...
    tri, tri_ht = fused_recurrent_oja2(
        q=q.clone(),
        k=k.clone(),
        v=F.normalize(v.clone(), p=2, dim=-1).to(dtype),
        beta=beta.clone(),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        for test in [
            ...
            (2, 1500, 4, 128, 0.1, 0.01, 0, torch.float16),
            (4, 2048, 8, 32, 0.1, 0.1, 0, torch.float16),
            (4, 4096, 8, 128, 0.1, 1, 0, torch.float16),
            (4, 4096, 4, 128, 0.1, 0.01, 0, torch.float16),
            (5, 3407, 16, 128, 1, 0.01, 0, torch.bfloat16),
            (4, 2048, 16, 128, 1, 0.1, 0, torch.float16),
            (4, 4096, 16, 128, 1, 0.01, 0, torch.float16),
        ]
    ]
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        for test in [
            ...
            # (2, 4096, 3, 64, 0.1, 1, 0, torch.float16),
            # (3, 4096, 4, 100, 1, 1, 0, torch.float16),
            # (4, 4096, 4, 128, 0.1, 1, 0, torch.float16),
            # (2, 4096, 4, 128, 0.1, 1, 0, torch.float16),
            # (4, 4096, 8, 64, 0.1, 1, 0, torch.float16),
            # (5, 3407, 16, 128, 1, 1, 0, torch.float16),
            # (4, 2048, 16, 128, 1, 1, 0, torch.float16),
            # (4, 4096, 16, 128, 1, 1, 0, torch.float16),
            
            # (4, 4096, 4, 128, 1, 0.1, 0, torch.float),
            # (1, 4096, 1, 64, 1, 1, 0, torch.float),
            # (2, 4096, 3, 60, 1, 0.1, 0, torch.float),
            # (2, 4096, 3, 64, 0.1, 0.1, 0, torch.float),
            # (3, 4096, 4, 100, 1, 0.1, 0, torch.float),
            # (4, 4096, 4, 128, 0.1, 0.1, 0, torch.float),
            # (2, 4096, 4, 128, 0.1, 10, 0, torch.float),
            # (4, 4096, 8, 64, 0.1, 1, 0, torch.float),
            # (5, 3407, 16, 128, 1, 10, 0, torch.float),
            # (4, 2048, 16, 128, 1, 1, 0, torch.float),
            # (4, 4096, 16, 128, 1, 10, 0, torch.float),
        ]
    ]
)
def test_chunk(
